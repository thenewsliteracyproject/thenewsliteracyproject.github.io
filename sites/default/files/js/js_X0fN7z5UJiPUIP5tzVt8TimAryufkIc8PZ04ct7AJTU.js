...
(function($) {
    ...
    var awpNlpGalleryInit = function(cx, s) {
        $('body').once('awp_nlp_gallery', function() {
            var gallery = $('#block-views-gallery-gallery, #block-views-gallery-block-1');
            $('.view-header', gallery).click(function() {
                $('.views-row a.colorbox').eq(0).trigger('click');
                return false;
            });
        });
    }
    
    Drupal.behaviors.awpNlpGallery = {
        attach: awpNlpGalleryInit
    }
...
(function($) {
    ...
    var awpNLPSearchInit = function(cx, s) {
        
        var block = $('#block-search-form', cx);
        var logo = $('#region-branding', cx);
        var form = $('#search-block-form', cx);
        var nav = $('nav.navigation', cx);
        var formTimer = [];
        ...
        var setTimer = function() {
            ...
            formTimer.push(timer);
        }
        
        var clearTimer = function(reset) {
            for (i in formTimer) {
                clearTimeout(formTimer[i]);
                delete formTimer[i];
            }
        ...
        block.click(function() {
            ...
            $('input', this).on('click keyup', function() {
                clearTimer(true);
            });
        });
    }
    
    Drupal.behaviors.awpNLPSearch = {
        attach: awpNLPSearchInit
    }
...
(function($) {
    ...
    var awpNlpMobileInit = function(cx, s) {
        $('#region-menu .navigation').bind('click touch', function(e) {
            console.log(Drupal.settings);
            if ($('body').is('.responsive-layout-mobile')) {
                $('body').toggleClass('display-menu');
            }
        });
        
        $('#region-menu .menu > li > a').bind('click touch', function(e) {
            if ($('body').is('.responsive-layout-mobile')) {
                if ($(this).parent().hasClass('mobile-hover') || !$(this).parent().hasClass('expanded')) {
                    return true;
                }
    
                $(this).parent().addClass('mobile-hover').siblings().removeClass('mobile-hover');
                return false;
            }
        });
    }
    
    Drupal.behaviors.awpNlpMobile = {
        attach: awpNlpMobileInit
    }
